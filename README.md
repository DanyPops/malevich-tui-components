...
| `Chip` | A single-line semantic/state indicator (`[In Progress]`, `✓ approved`) -- bracket/chevron/plain shape, optional icon, no numeric value (that's `Badge`'s job). `formatChip` is the underlying pure formatter, usable inline without the Component wrapper. |
