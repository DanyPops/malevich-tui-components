...
describe("Spinner", () => {
	...
	it("accepts custom frames and interval, for a host that wants a different animation", async () => {
		const spinner = new Spinner({ frames: ["a", "b", "c"], intervalMs: 10 });
		expect(spinner.glyph()).toBe("a");
		spinner.tick();
		expect(spinner.glyph()).toBe("b");
	});
});
